...
static int fsal_check_access_acl(struct user_creds *creds,   /* IN */
				 fsal_aceperm_t v4mask,  /* IN */
				 fsal_attrib_list_t * p_object_attributes   /* IN */ );

static int fsal_check_access_no_acl(struct user_creds *creds,   /* IN */
				    fsal_accessflags_t access_type,  /* IN */
				    fsal_attrib_list_t * p_object_attributes /* IN */ );


static fsal_boolean_t fsal_check_ace_owner(fsal_uid_t uid, struct user_creds *creds)
{
  return (creds->caller_uid == uid);
}

static fsal_boolean_t fsal_check_ace_group(fsal_gid_t gid, struct user_creds *creds)
{
  int i;
 ...
 * consolidate old api.  deprecated in new api.
 */
				...
				fsal_attrib_list_t * p_object_attributes /* IN */ )
{
  ...
  creds.caller_uid = p_context->credential.user;
  creds.caller_gid = p_context->credential.group;
  creds.caller_glen = p_context->credential.nbgroups;
  creds.caller_garray = &p_context->credential.alt_groups;
  if(p_object_attributes)
    {
      if(p_object_attributes && p_object_attributes->acl &&
	 IS_FSAL_ACE4_MASK_VALID(access_type))
        {
          retval = fsal_check_access_acl(creds, FSAL_ACE4_MASK(access_type),
					 p_object_attributes);
	}
      else  /* Use mode to check access. */
        {
	  retval = fsal_check_access_no_acl(creds, FSAL_MODE_MASK(access_type),
					    p_object_attributes);
	}
    }
  else
    {
      ...
      attribs.owner = p_buffstat.st_uid;
      attribs.group = p_buffstat.st_gid;
      attribs.mode = unix2fsal_mode(p_buffstat.mode);
      retval = test_access_creds(&creds, access_type, &attribs);
    }
  if(retval)
    ReturnCode(ERR_FSAL_NO_ERROR, 0);
  else
