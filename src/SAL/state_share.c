                               ...
                               state_status_t        * pstatus)
{
  ...
  if(status != STATE_SUCCESS)
    {
      V_w(&pentry->lock);
      LogDebug(COMPONENT_STATE, "Share conflicts detected during add");
                                   ...
                                   state_status_t        * pstatus)
{
  ...
  if(status != STATE_SUCCESS)
    {
      V_w(&pentry->lock);
      LogDebug(COMPONENT_STATE, "Share conflicts detected during upgrade");
      *pstatus = STATE_STATE_CONFLICT;
      return *pstatus;
    }
                                             ...
                                             int use_mutex)
{
  ...
  if((pstate_data->share.share_access & 1) != 0 &&
  ...
  if((pstate_data->share.share_access & 2) != 0 &&
  ...
  if((pstate_data->share.share_deny & 1) != 0 &&
  ...
  if((pstate_data->share.share_deny & 2) != 0 &&
...
static void state_share_update_counter(cache_entry_t * pentry,
                                       ...
                                       int new_deny)
{
  int access_read_inc  = ((new_access & 1) != 0) - ((old_access & 1) != 0);
  int access_write_inc = ((new_access & 2) != 0) - ((old_access & 2) != 0);
  int deny_read_inc    = ((new_deny   & 1) != 0) - ((old_deny   & 1) != 0);
  int deny_write_inc   = ((new_deny   & 2) != 0) - ((old_deny   & 2) != 0);
